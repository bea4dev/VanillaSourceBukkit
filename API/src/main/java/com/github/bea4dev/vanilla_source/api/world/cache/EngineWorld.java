...
import com.github.bea4dev.vanilla_source.api.world.IWorld;
...
public interface EngineWorld extends IWorld {
    ...
    default @Nullable EngineRayTraceResult rayTrace(@NotNull Vector startPosition, @NotNull Vector direction, double distance, @NotNull CollideOption collideOption) {
        ...
        if (blockResult == null && entityResult == null) {
            ...
            if (blockResult.hitPosition().distanceSquared(startPosition) < entityResult.hitPosition().distanceSquared(startPosition)) {
                return blockResult;
            } else {
                return entityResult;
            }
    ...
    default @Nullable EngineRayTraceResult rayTraceBlocks(@NotNull Vector startPosition, @NotNull Vector direction, double distance, @NotNull CollideOption collideOption) {
        if (distance < 1.0) {
            ...
            for (double index = 0; index <= distance; index += 1.0) {
                ...
                if (result.hitPosition().distanceSquared(startPosition) > distanceSquared) continue;

                if (nearestResult == null) {
                    nearestResult = result;
                } else if (nearestResult.hitPosition().distanceSquared(startPosition) > result.hitPosition().distanceSquared(startPosition)) {
                    nearestResult = result;
                }
            }
    ...
    static @Nullable EngineRayTraceResult rayTraceForCollisionList(@NotNull Vector startPosition, @NotNull Vector direction, double distance, Collection<EngineBoundingBox> boxList, @NotNull CollideOption collideOption) {
